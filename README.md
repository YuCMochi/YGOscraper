...
| 版本 | 說明 |
|------|------|
| **v0.4.3** | 修正健康檢查 probe URL（salix5 卡圖目錄路徑、露天拍賣 API base URL） |
| **v0.4.2** | 版本號統一管理：`server.py` 為單一真相來源，前端動態讀取 `/api/version` |
| **v0.4.1** | 前端標籤與 Tooltip 調整：純 CSS tooltip 分三行顯示稀有度及卡包出處 |
